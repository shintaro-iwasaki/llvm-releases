...
if [ x"$ARCH" == x"arm64" ] && [ x"$PLATFORM" == x"ubuntu-latest" ]; then
  ...
  cp bin/mlir-tblgen $BUILD_DIR/LLVM_INSTALL/bin/mlir-tblgen
  cp bin/llvm-tblgen $BUILD_DIR/LLVM_INSTALL/bin/llvm-tblgen
  cp bin/mlir-linalg-ods-yaml-gen $BUILD_DIR/LLVM_INSTALL/bin/mlir-linalg-ods-yaml-gen
  cp bin/mlir-pdll $BUILD_DIR/LLVM_INSTALL/bin/mlir-pdll
  popd
fi
